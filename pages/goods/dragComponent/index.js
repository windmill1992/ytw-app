...
Component({
	...
	properties: {
		...
		spec: {type: Boolean, value: false},                  // 是否是规格页面
		del: {type: Boolean, value: false},                  // 是否显示删除
		delx: {type: Boolean, value: false},                  // 是否展示右上角删除  X
		change: {type: Boolean, value: false},                  // 是否显示换图文字  换
		scrollTop: {type: Number, value: 0},                    // 页面滚动高度
		isMain: {type: Boolean, value: false}                    // 页面滚动高度
	},
	data: {
		/* 未渲染数据 */
	...
	methods: {
		...
		updateList(data, vibrate = true) {
			...
			let list = data.map((item, index) => {
				...
				if (data.length >= 4 && item.slot == 'plus') {
					...
					if (spec) {
						item.tranY = `${(Math.floor(item.sortKey / this.data.columns) - 0 + Math.ceil(data.length / 3 - 1) * 0.358 ) * 100}%`;
					} else {
						item.tranY = `${(Math.floor(item.sortKey / this.data.columns) - 0 + Math.ceil(data.length / 4 - 1) * 0.32 ) * 100}%`;
					}
				} else {
					item.tranX = `${(item.sortKey % this.data.columns) * 100}%`;
		...
		editSpec:function(e){
			...
			if (e.detail.value != '') {
				this.triggerEvent('editSpec', obj, {})
			}
		},
		changeImg:function(e){
			...
			if (this.data.spec) {
				...
				params = {
					id: e.currentTarget.dataset.info.data.dragId,
					type: this.data.isMain
				}
			}
			// console.log(params)
			// return
			this.triggerEvent('changeImg', params, {})
		},
		pTu:function(e){//p图
			this.triggerEvent('pTu',e.currentTarget.dataset.info.data,{})
		}
	},
	ready() {
		this.init();
